  ...
  data.forEach((producto)=>{
    const productContainer = document.getElementById("product-container");
      const card = document.createElement('card');
        card.innerHTML += `<h3 class="text-center text-n">${producto.nombre}</h3>
                            <h5 class="text-center"> Nro:${producto.id}</h5> 
                            <img src = "${producto.img}" class="img">
                            <h5 class="text-center text-p">$${producto.precio}</h5>
...
function renderizarCarrito(){
  ...
  .then((data)=>{
     carritoSinDuplicados.forEach((item) => {
        ...
        BotonB.classList.add("btnBorrar")
...
formulario.addEventListener('submit', (e)=>{
  ...
  (fname.value === "" || femail.value === "") ? swal(`Rellenar todos los campos`): swal(`El formulario fue enviado con éxito. Gracias ${fname.value}`);
});
